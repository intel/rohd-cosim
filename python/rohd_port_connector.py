...
async def launch_on_port(
    ...
    def run_dart_thread() -> subprocess.Popen:
        ...
        with    open(f'{log_name}.stdout.log', 'w', encoding="utf-8") as out,\
                open(f'{log_name}.stderr.log', 'w', encoding="utf-8") as err:
            print('Dart process running, waiting for it to finish...')
            dart_process : subprocess.Popen = subprocess.Popen(
                dart_command(connector.socket_port),
                stdout=out,
                stderr=err
            )
    ...
    logging.getLogger('cocotb.regression').addHandler(RohdLoggerHandler(connector=connector))
    ...
    try:
        ...
        if do_dart_launch:
            ...
            if enable_logging: print('Port connector loop has completed!')
            if enable_logging: print(f'Dart thread alive? -- {dart_process.poll() is None}')
            if enable_logging: print(f'Connection task done? -- {connection_task.done()}')

            try:
                dart_process.wait(timeout=2)
                if enable_logging: print('Dart process completed gracefully.')

            except subprocess.TimeoutExpired:
                print('Dart process is still running, attempting a graceful shutdown...')
                connector.shutdown()
                sleep(2)
    ...
    if enable_logging: print('launch_on_port completed!')
