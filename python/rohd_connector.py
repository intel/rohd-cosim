...
class RohdConnector:
    ...
    def __init__(self,
        ...
        if self.enable_logging: cocotb.log.setLevel(logging.DEBUG)
    ...
    def connect_to_socket(self):
        ...
        """
        print(f'ROHD COSIM SOCKET:{self.socket_port}', flush=True) # critical for connection
        ...
        try:
            ...
            self._error(f'Socket connection timed out after {self.dart_connect_timeout} seconds!')

    def _sock_send(self, message: str):
        try:
            self.sock.send((f"@{self.time}:" + message + '\n').encode())
        except Exception as exception:
            self._error(f'Encountered error when sending message "{message}": {exception}')
    ...
    async def listen_for_stimulus(self, name_to_signal_map):
        ...
        if self.enable_logging: print('Listening for stimulus...')
        self.continue_listen = True
        pending_message = ''
        ...
        while self.continue_listen:
            ...
            try:
                ...
                if self.enable_logging:
                    print('Detected socket shutdown, ending cosim execution!', flush=True)
                self.sock.shutdown(socket.SHUT_RDWR)
                self.sock.close()
                self.continue_listen = False
                break
            ...
            if (
                    self.last_message_received_time is not None and
                    (time.time() - self.last_message_received_time) > self.dart_connect_timeout
            ):
                self._error(
            ...
            if len(ready_to_read) > 0:
                ...
                try:
                    full_message = pending_message + self.sock.recv(1024).decode("ascii")
                except Exception as exception:
                    print(f'Encountered exception when reading from socket: {exception}')
                ...
                for message in split_message:
                    ...
                    if self.enable_logging: print(f'Received message: {message}')
                    if message.startswith('TICK:'):
                        ...
                        if self.enable_logging: print('Finishing listening for stimulus')
                        self.shutdown()
                        break
                    else:
                        self._error(f'Unknown message received: {message}')

        if self.enable_logging: print('Listener loop ended.')
    ...
    async def _tick(self, new_time: int): #ns
        ...
        if self.enable_logging: print(f'>>> current time: {self.time}, new_time: {new_time}')
